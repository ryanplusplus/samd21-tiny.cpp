...
static class : public IAsyncSpi {
 public:
  ...
  void configure_read_channel(const void* buffer, uint16_t buffer_size)
  {
    ...
    d.SRCADDR.bit.SRCADDR = reinterpret_cast<uintptr_t>(&SERCOM0->SPI.DATA.reg);
  ...
  void configure_write_channel(const void* buffer, uint16_t buffer_size)
  {
    ...
    d.DSTADDR.bit.DSTADDR = reinterpret_cast<uintptr_t>(&SERCOM0->SPI.DATA.reg);
    d.DESCADDR.bit.DESCADDR = 0;
...
tiny::IAsyncSpi& Sercom0Pa09Pa10Pa11AsyncSpi::get_instance(
  bool cpol,
  bool cpha,
  bool msb_first,
  uint32_t baud)
